...
// Global state
//
static const uint256 hashGenesisBlockOfficial("0x000005aef7f601a0e5b5f17619735819d4250af7f0282954333e634335b35c9e");
static const uint256 hashGenesisBlockTestNet ("0x00000e9b0e112f49336d281a8defe0930b720fa03a7176b5fed41eb6663cf19a");
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // YBCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // YBCoin: enforce minimum output amount
...
// YBCoin: minter's coin stake is rewarded based on coin age spent (coin-days)
...
bool CBlock::AcceptBlock()
{
    ...
    // YBCoin: check switch from proof of work to proof of stake
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "YBCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "YBCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Created in China, For the world 2014-11-17 17:58";
        unsigned int nTimeGenesis=1416218298;
        unsigned int nNonceGenesis=275663;

        if (fTestNet)
        {
            pszTimestamp="Created in China, For the world 2014-11-17";
            nTimeGenesis=1416218298;
            nNonceGenesis=42103;
        }
        ...
        printf("YBCoin Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End YBCoin Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0x7e5904d103bd82a0819af09ed0e35f3099885c3038bb5631def64c1d7f4ac8c5"));
        else
            assert(block.hashMerkleRoot == uint256("0x20219a0e187670a32a3bbe5a325d732377408833795a320c8b24de186d9db9f7"));
...
string GetWarnings(string strFor)
{
    ...
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/YBCoin/YBCoin/issues).";
    }
    ...
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/YBCoin/YBCoin/issues).";
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // YBCoin: immediate retry during initial download
            ...
            if (nNonceFound != (unsigned int) -1)
            {
                ...
                if (hash <= hashTarget)
                {
                    ...
                    /*if(!fTestNet)
                    {
                        if(nLastBlockTime ==0)
                        {
                            nLastBlockTime = GetTimeMillis();
                        }
                        else
                        {
                            int time = 20000 - (GetTimeMillis() - nLastBlockTime);
                            if(time >0)
                                Sleep(time);
                            nLastBlockTime = GetTimeMillis();
                        }
