...
// YBCoin note: this is only useful during the initial proof-of-work 'IPO' phase of the network
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid YBCoin address");
...
Value edittaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "edittaccount <YBCoinaddress> <account>\n"
            "Sets the account associated with the given address.");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid YBCoin address:")+s.name_);
...
Value distribute(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        {
            Object obj;
            obj.push_back(Pair("ybcoin_address", distribution.GetPeershareAddress().ToString()));
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Portfolio encrypted; YBCoin server stopping. Please restart server to run with encrypted portfolio";
}
